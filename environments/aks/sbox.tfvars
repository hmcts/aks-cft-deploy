clusters = {
  "00" = {
    kubernetes_cluster_version        = "1.32"
    kubernetes_cluster_ssh_key        = "ssh-rsa "
    enable_user_system_nodepool_split = true
    project_acr_enabled               = true

    enable_automatic_channel_upgrade_patch = true

    system_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 2
      max_nodes = 4
    }

    linux_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 4
      max_nodes = 10
      max_pods  = 30
    }

    availability_zones = ["1"]

    node_os_maintenance_window_config = {
      frequency  = "Daily"
      start_time = "16:00"
      is_prod    = false
    }
  },

  "01" = {
    kubernetes_cluster_version        = "1.32"
    kubernetes_cluster_ssh_key        = "ssh-rsa "
    enable_user_system_nodepool_split = true
    project_acr_enabled               = true

    enable_automatic_channel_upgrade_patch = true

    system_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 2
      max_nodes = 4
    }

    linux_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 4
      max_nodes = 10
      max_pods  = 30
    }

    availability_zones = ["1"]

    node_os_maintenance_window_config = {
      frequency  = "Daily"
      start_time = "16:00"
      is_prod    = false
    }
  }

  # Team Blue POC cluster - intentionally on older minor version (1.28) for upgrade testing exercises
  "02" = {
    kubernetes_cluster_version        = "1.30"
    kubernetes_cluster_ssh_key        = "ssh-rsa "
    enable_user_system_nodepool_split = true
    project_acr_enabled               = true

    # Disable automatic patch channel so manual upgrade path can be rehearsed
    enable_automatic_channel_upgrade_patch = false

    system_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 2
      max_nodes = 3
    }

    linux_node_pool = {
      vm_size   = "Standard_D4ds_v5"
      min_nodes = 2
      max_nodes = 6
      max_pods  = 30
    }

    availability_zones = ["1"]
  }
}

autoShutdown = true
