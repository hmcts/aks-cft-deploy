...
for cluster in ${6}; do 
  ...
  [[ $3 =~ ^(sbox-ptl|sbox|preview|ithc|perftest|aat|demo|)$ ]] && (./scripts/disable-container-insights.sh "$@" || error_exit "ERROR: Unable to disable container application insights")
