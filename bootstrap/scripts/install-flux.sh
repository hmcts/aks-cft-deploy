...
if [ "${ENVIRONMENT}" == "ptlsbox" ]; then
  CLUSTER_ENV="sbox-intsvc"
elif [ "${ENVIRONMENT}" == "ptl" ]; then
...
cat <<EOF
    ...
    patches:
      - path: https://raw.githubusercontent.com/hmcts/cnp-flux-config/refs/heads/master/apps/admin/aad-pod-id/nmi-patch.yaml
      - path: https://raw.githubusercontent.com/hmcts/cnp-flux-config/refs/heads/master/apps/admin/aad-pod-id/mic-patch.yaml
EOF
...
# Apply
    ...
    kubectl apply -f https://raw.githubusercontent.com/hmcts/cnp-flux-config/refs/heads/master/apps/admin/aad-pod-id/mic-exception.yaml 2>&1 | filter_kubectl_warnings
    kubectl apply -f https://raw.githubusercontent.com/hmcts/cnp-flux-config/refs/heads/master/apps/kube-system/aad-pod-id/mic-exception.yaml 2>&1 | filter_kubectl_warnings
}
...
function install_aso {
  ...
  download_files "https://api.github.com/repos/hmcts/cnp-flux-config/contents/apps/azureserviceoperator-system/cert-manager" "${TMP_DIR}/cert-manager"

  # Download files in aso directory
  echo "Deploying ASO - Downloading aso"
  download_files "https://api.github.com/repos/hmcts/cnp-flux-config/contents/apps/azureserviceoperator-system/aso" "${TMP_DIR}/aso"

  # Download aso-controller-settings.yaml
  echo "Deploying ASO - Downloading aso-controller-settings"
  curl -sL "https://raw.githubusercontent.com/hmcts/cnp-flux-config/refs/heads/master/apps/azureserviceoperator-system/${CLUSTER_ENV}/base/aso-controller-settings.yaml" -o "${TMP_DIR}/aso-controller-settings.yaml"
...
patches:
  - path: ${FLUX_CONFIG_URL}/apps/flux-system/${CLUSTER_ENV}/${CLUSTER_NAME}/kustomize.yaml
EOF
