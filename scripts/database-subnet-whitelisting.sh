...
JSONPATH="[?(contains(type, 'Microsoft.DBforPostgreSQL/servers') && contains(name, 'perftest'))].name"
POSTGRES_LIST=$(az resource list --query "${JSONPATH}" --subscription="" -o tsv | sort -u)
...
do
    ...
    *"-perftest-clone"*)
        rg=${pg//-perftest-clone/-data-perftest}
        ;;
    
    *"-perftest"*)
        rg=${pg//-perftest/-data-perftest}
        ;;

    esac
    if [ "$(az postgres server vnet-rule list --resource-group $rg --server-name $pg -o tsv)" ]; 
    then 
        echo "Skipping this as it already has vnet rules"; 
    else 
