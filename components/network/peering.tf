...
module "vnet_peer_hub_prod" {
  ...
  initiator_peer_name = var.env == "ptl" ? "${local.hub["prod"][each.key].peering_name}-prod" : local.hub["prod"][each.key].peering_name
...
module "vnet_peer_hub_nonprod" {
  ...
  initiator_peer_name = var.env == "ptl" ? "${local.hub["prod"][each.key].peering_name}-nonprod" : var.env == "aat" ? "${local.hub["prod"][each.key].peering_name}-nonprod" : local.hub["prod"][each.key].peering_name
